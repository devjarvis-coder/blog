...
def states(request,s):
    print(s)
    a=Categorie.objects.get(Places__exact=s)
    print(a)
    b=Blog.objects.filter(places__exact=a)
    return render(request,'career.html',{'a':a,'b':b})
        # states = Blog.objects.filter(title__contains=s)
        # return render(request,'show.html',{'data':states})
